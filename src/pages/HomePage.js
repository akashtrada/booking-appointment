...
import SubHeader from '../components/templates/SubHeader/SubHeader';
import Calendar from '../components/organisms/Calendar/Calendar';
import CreateBookingDrawer from '../components/organisms/CreateBookingDrawer/CreateBookingDrawer';
import FilterMenu from '../components/organisms/FilterMenu/FilterMenu';
...
export default function HomePage() {
  ...
  const filteredBookingsByTherapist = useMemo(() => {
    ...
    if (!hasStatusFilter && !hasResourceFilter) return bookingsByTherapist;

    const result = {};
    Object.entries(bookingsByTherapist).forEach(([tid, bookings]) => {
      const filtered = bookings.filter((b) => {
        if (hasStatusFilter && statusFilters.includes(b.status)) return false;
      ...
      if (filtered.length > 0) result[tid] = filtered;
    });
    return result;
  ...
  const handleBookingDrop = useCallback(async ({ booking, newStartMinute }) => {
    ...
    const newEndStr = `${String(Math.floor(newEndMin / 60) % 24).padStart(2, '0')}:${String(newEndMin % 60).padStart(2, '0')}`;
  ...
  return (
    <>
      ...
      <CreateBookingDrawer />
      <FilterMenu anchorEl={filterAnchorEl} onClose={() => setFilterAnchorEl(null)} />
    </>
  );
}
